...
const BUCKET: &str = "rcm-road-img";
// const MESSAGE: &str = "I want to go to S3";
const REGION: &str = "us-east-1";

fn main() {

    // let access_key = String::from("AKNDAIKDNA");
    // let secret_key = String::from("ADJKANDKAND")

    let region = REGION.parse().unwrap();
    //     Create Bucket in REGION for BUCKET
    // let credentials: Credentials = Credentials::new(Some(access_key), Some(secret_key), None, None);
    ...
    // bucket.
    let (_, code) = bucket.put_object("test", content.as_ref(), "text/plain").unwrap();
    assert_eq!(200, code);

    // let camera_data = parse_xml::parse_cameras("camera_data_cache.xml");
    // fetch::img(auth::USER_DATEX, auth::PASS_DATEX, camera_data);
